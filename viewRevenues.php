...
<html>
  ...
  <body style='background-color: #F0EEEE;'>
    ...
    <?php
      ...
      $sql1 = "SELECT SUM(quatity_sold*price_per_unit) as sum_product FROM product";
      $result = mysqli_query($dbconnect, $sql1);
      $sum_product = mysqli_fetch_assoc($result)['sum_product'];
      
      $sql2 = "SELECT SUM(quatity_sold*price_per_unit) as sum_medicine FROM product WHERE pro_cat = 'medicine'";
      $result = mysqli_query($dbconnect, $sql2);
      $sum_medicine = mysqli_fetch_assoc($result)['sum_medicine'];

      $sql3 = "SELECT SUM(quatity_sold*price_per_unit) as sum_healthcare FROM product WHERE pro_cat = 'healthcare'";
      $result = mysqli_query($dbconnect, $sql3);
      $sum_healthcare = mysqli_fetch_assoc($result)['sum_healthcare'];

      $sql4 = "SELECT SUM(quatity_sold*price_per_unit) as sum_babycare FROM product WHERE pro_cat = 'babycare'";
    ...
    <?php
    echo  "<div class='card'>
            <h1 style='text-align: center; margin-top: 5px; margin-bottom: 25px; background-color: lightblue; width: 100%;'>REVENUES</h1>
            <h2 style='text-align: left; padding-left: 15px; margin-top: 10px; background-color: lightblue; height: 3rem;'> <i style='color: navy;' class='fas fa-dollar-sign'></i>Total Revenues  : $sum_product/-</h2>

            <h2 style='text-align: left; padding-left: 15px; margin-top: 25px; background-color: lightblue;'> <i style='color: red;' class='fas fa-medkit'></i> Medicines   :   $sum_medicine/-</h2>
            <h4 style='background-color: lightblue; height: 2rem; width: 100%; margin-bottom: 25px;color: red'> $medicine_bar% of Total Revenues </h4>
            <h2 style='text-align: left; padding-left: 15px; margin-top: 25px; background-color: lightblue;'> <i style='color: green;' class='fas fa-dumbbell'></i> Healthcare  :   $sum_healthcare/-</h2>
            ...
            <h2 style='text-align: left; padding-left: 15px; margin-top: 25px; background-color: lightblue;'> <i style='color: #115BFE;' class='fas fa-baby-carriage'></i> Babycare    :   $sum_babycare/-</h2>
