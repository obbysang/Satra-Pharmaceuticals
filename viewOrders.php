  ...
  <body style='background-color: #F0EEEE;'>
    ...
    <div class="main">
      ...
      <div class="container" style='margin-top: 10px; padding: 10px;'>
        <div class="row">
          <!-- The Order Table -->
          <div class="col-md-12" style='margin-left: -10px; padding: 0px;'>
            <h5 style='background-color: #962EB4; color: white; text-align: center; margin-bottom: 0px;'> Currently Acitve Users</h5>
            <table class="table-responsive">
              <table class="table table-bordered table-striped" style='margin-top: 0px; margin-bottom: 0px;'>
                <thead>
                  <tr style='background-color: #022A33; color: white;'>
                    ...
                    <th><p style="font-size: 20px; text-weight: bold; text-align: center;">Products Quantity</p></th>
                    <th><p style="font-size: 20px; text-weight: bold; text-align: center;">Total Bill</p></th>
                    <th><p style="font-size: 20px; text-weight: bold; text-align: center;">Placed On</p></th>
                    <th><p style="font-size: 20px; text-weight: bold; text-align: center;">Action</p></th>
                  </tr>
                </thead>
                <tbody>
                  <?php
                    ...
                    if(isset($_GET['ord_id'])){
                      $ord_id = $_GET['id'];
                      $sqlN = "UPDATE orders SET order_status='Delivered' WHERE order_id=$ord_id";
                      mysqli_query($dbconnect, $sqlN);
                    }
                    ...
                    while($row = mysqli_fetch_array($result)){
                      if($i%2 == 0){$bc_color = '#DDC7E3';}
                      else{$bc_color = '#D499E5';}
                      $id = $row['order_id'];
                      $email = $row['ordered_by_user_email'];
                      $qty = $row['order_total_quantity'];
                      $amnt = $row['order_total_amouont'];
                      $placed = $row['order_date'];
                      $status = $row['order_status'];
                  ?>
                      <tr style='background-color: <?php echo $bc_color; ?>'>
                        <form action="viewOrders.php" method='GET'>
                          <input type='hidden' name='id' value='<?php echo $id?>'> <!-- this input box will store the id of the customers-->
                          <td><p style="margin-top: 23px; color: black; font-size: 20px; text-weight: bold; text-align: center;"> <?php echo $id; ?> </p></td>
                          <td><p style="margin-top: 23px; color: black; font-size: 20px; text-weight: bold; text-align: center;"><?php echo $email;?></p></td>
                          <td><p style="margin-top: 23px; color: black; font-size: 20px; text-weight: bold; text-align: center;"><?php echo $qty;?></p></td>
                          <td><p style="margin-top: 23px; color: black; font-size: 20px; text-weight: bold; text-align: center;"><?php echo $amnt;?></p></td>
                          <td><p style="margin-top: 23px; color: black; font-size: 20px; text-weight: bold; text-align: center;"><?php echo $placed;?></p></td>
                          <?php
                            if($status == 'placed'){
                              echo "<td style='text-align: center;'>
                                      <input formaction='viewOrders.php' type='submit' name='ord_id' style='margin-top: 1.5rem;' class='btn btn-success btn-pill' value='Deliver'>
                                    </td>";
                            }
                            else{
                              echo "<td>
                                        <p style='margin-top: 23px; color: green; font-size: 20px; text-weight: bold; text-align: center; margin-bottom: 0px;'>Delivered</p>
                                    </td>"; //<a href='viewOrders.php' type='submit' name='cancel' style='margin-top: -6px; color: red; margin-left: 28px;'>Cancel</a>
                            }
                          ?>
